...
Root-to-Shoot Ratio
```{r, echo = T, results='show'}
mod_rts <- lmer(rts ~ Origin + Water*Nitrogen + (1 | Parent:Origin), data = bsdata_complete)
...
RGR
```{r, echo = T, results='show'}
mod_rgr <- lmer(kwrgr ~ Origin + Water*Nitrogen + (1 | Parent:Origin), data = bsdata_complete)
...
final_model <- get_model(step_result)
final_model
#formula = kwrgr ~ Origin + Water * Nitrogen + (1 | Parent:Origin)
```
SRL
```{r, echo = T, results='show'}
mod_srl <- lmer(srl ~ Origin + Water*Nitrogen + (1 | Parent:Origin), data = bsdata_complete)
...
final_model <- get_model(step_result)
final_model
#formula = srl ~ Origin + Water * Nitrogen + (1 | Parent:Origin)
```
RTD
```{r, echo = T, results='show'}
mod_rtd <- lmer(rtd ~ Origin + Water*Nitrogen + (1 | Parent:Origin), data = bsdata_complete)
