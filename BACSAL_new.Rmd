...
bsdata %>% 
  ...
  mutate(rts = RootMassg / ShootMassg ) %>% 
  ggplot() +
    aes(x = factor(Origin, level = c('BOYD', 'ELFR', 'COAL')), #reorders sites to be in order of precipitation
        y = rts, fill = Nitrogen) +
...
#Checking stat_sig using unpaired t.test
rts_test <- bsdata %>% 
  select(Code, Origin, Treatment, Water, Nitrogen, RootMassg, ShootMassg) %>% 
  mutate(rts = RootMassg / ShootMassg )

#Applying Welch's t-test for unpaired samples
# WTtest <- lapply(split(rts_test, factor(rts_test$Origin)), 
#       function(x)t.test (data = rts_test, rts ~ Nitrogen, paired = F))
# WTest <- rts_test %>% 
#   filter()
#   
#   t.test(rts ~ Nitrogen, data = rts_test)
