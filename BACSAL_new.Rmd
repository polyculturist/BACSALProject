...
```{r RTS Linear Regression, echo=T, results='show'}
#Dividing data into train and test set
NoNAs <- bsdata_complete %>% 
  drop_na(rts)

index <- createDataPartition(NoNAs$rts, p=.7, list = F)
train <- bsdata_complete[index, ]
test <- bsdata_complete[-index, ]

# Checking the dim of train
dim(train)

# Training model
lmModel <- lm(rts ~ AIKoppen  + Water + Nitrogen + Sex , data = train) 
#AIKoppen instead of Origin
...
#Make figure for supplement on fine:coarse root
l
...
#rdpi_raw <- rdpi(bsdata_complete, Origin, Lengthcm, Code)
#???
