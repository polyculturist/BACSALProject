...
#     library(ggplot2)
#     library(sciplot)
# # Create the object (an empty data frame) that will store the results of RDPI for rts
# RDPI <- data.frame(Origin = character(0), value = numeric(0))
# #view(RDPI)
# 
# # Since we need to compute everything per Origin, we make a 'for' loop
# levels_bsdata_complete <- levels(bsdata_complete %>% pull({{Origin}}) %>% droplevels())
#     for (a in levels_bsdata_complete) {
#         # subset the data for a given species
#         data_Origin <- bsdata_complete %>% filter({{Origin}} == a)
# 
#         RDPI_rts <- rdpi_matrix(data_Origin, {{rts}}, {{Treatment}})
# 
#         RDPI_Origin <- data.frame(Origin = as.character(a),
#                               rdpi = RDPI_rts)
# 
#         RDPI <- rbind(RDPI, RDPI_Origin) %>%
#             mutate(Origin = as.factor(Origin))
#     
# }
#   # A table with summary statistics for each Origin
#     summary <- RDPI %>%
#         group_by(Origin) %>%
...
rdpi_Lengthcm <- rdpi(bsdata_complete, Origin, kwrgr, Treatment)
